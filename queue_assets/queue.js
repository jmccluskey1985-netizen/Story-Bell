...
(function(){
  ...
  function cfg(){ return { STILL_FIRST: window.SBQ_STILL_FIRST || '', COUNTDOWN_URL: window.SBQ_COUNTDOWN_URL || '' }; }
  ...
  function setMode(active){
    if(active){
      bodyOn('sbq-mode');
      var qm=$('#queue-mode-toggle'); if(qm){ qm.classList.add('active'); var l=qm.querySelector('.label'); if(l) l.textContent='Done'; }
      updateToggle(true);
    }else{
      bodyOff('sbq-mode');
      var qm2=$('#queue-mode-toggle'); if(qm2){ qm2.classList.remove('active'); var l2=qm2.querySelector('.label'); if(l2) l.textContent='Create Queue'; }
      updateToggle(false);
    }
    syncBadges();
  }
  ...
  function revealToggleOnce(){
    ...
    var w = $('#sbq-toggle-wrap'); if(w) w.style.display = 'inline-block';
  }
  ...
  function ensureOverlay(title){
    var ov = $('#sbq-next-overlay'); if(ov) return ov;
    ov = document.createElement('div');
    ov.id = 'sbq-next-overlay';
    ov.innerHTML = '<div class="sbq-next-title"></div><div class="sbq-next-count"></div><button class="sbq-next-btn" aria-label="Pause auto-continue" title="Pause">⏸</button>';
  ...
  function updateOverlayText(sec, title){
    ...
    var tEl = ov.querySelector('.sbq-next-title'); var cEl = ov.querySelector('.sbq-next-count');
    if(title) tEl.textContent = title;
    cEl.textContent = 'Continuing in… ' + sec;
  }

  function playCountdownCue(){
    var url = cfg().COUNTDOWN_URL; if(!url) return;
    try{
      if(window.__sbqCountdownAudio){ try{ window.__sbqCountdownAudio.pause(); }catch(_){ } }
      var a = new Audio(url); a.volume=1.0; a.currentTime=0; a.play().catch(function(_){});
  ...
  function ensureBadge(el, n){
    ...
    if(!b){ b = document.createElement('div'); b.className='sbq-badge'; el.insertAdjacentElement('afterbegin', b); }
  ...
  function syncBadges(){
    ...
    collectCards().forEach(function(card){
      var info = extract(card); var n = map.get(info.id);
  ...
  function renderList(){
    ...
    if(!q.length){
      ...
      ['#queue-prev','#queue-next','#queue-clear'].forEach(function(sel){ var b=$(sel); if(b) b.disabled=true; });
      return;
    }
    ...
    enableDnd(list);
    ['#queue-prev','#queue-next','#queue-clear'].forEach(function(sel){ var b=$(sel); if(b) b.disabled=false; });
  }
  ...
  document.addEventListener('click', function(e){
    ...
    var info = (function(el){
      var a = (el.tagName==='A') ? el : el.querySelector('a[href]');
      var href = a ? a.getAttribute('href') : (el.getAttribute('href')||'#');
      var id = (el.getAttribute('data-story-id') || (function(h){
        try{ var u=new URL(h,location.href); var fn=u.pathname.split('/').pop()||''; return fn.replace(/\.[^.]+$/,'').toLowerCase(); }
        catch(_){ var fn2=(h||'').split('/').pop()||''; return fn2.replace(/\.[^.]+$/,'').toLowerCase(); }
      })(href));
      var titleNode = (
        el.getAttribute('data-title')
        || (el.querySelector('[data-title]') && el.querySelector('[data-title]').getAttribute('data-title'))
      ...
      var title = (titleNode && titleNode.trim()) || (function(s){
        try{ s = decodeURI(s||''); }catch(_){}
        s = (s||'').replace(/\.[^.]+$/,'').replace(/[_\-]+/g,' ').replace(/\s+/g,' ').trim();
        if(!s) return s;
        return s.split(' ').map(function(w){ return w.charAt(0).toUpperCase() + w.slice(1); }).join(' ');
    ...
    info.title = (info.title && info.title.trim()) || info.title;
    saveQ(q); syncBadges();
  }, true);
  ...
  function bindAutoAdvanceOnAudio(){
    ...
    var q   = loadQ(); var idx = q.findIndex(function(x){ return x.id === id; });
    if(idx < 0) return;

    var main = document.querySelector('audio[data-sbq-main]') || Array.from(document.querySelectorAll('audio')).find(function(a){ return a.offsetParent !== null; }) || document.querySelector('audio');
    if(!main || main.__sbqAA) return;
    main.__sbqAA = true;

    main.addEventListener('play', function(){ clearNavAndCountdown(); });
    main.addEventListener('ended', function(){
      var curQ = loadQ(); var curIndex = curQ.findIndex(function(x){ return x.id === id; });
      ...
      window.__sbqCountdownStarter = setTimeout(function(){
        playCountdownCue(); startCountdown(COUNTDOWN_DURATION_S, title);
  ...
  function scheduleNavigation(delayMs){
    ...
    window.__sbqNavTimer = setTimeout(function(){
      var id  = (location.pathname.split('/').pop()||'').replace(/\.[^.]+$/,'').toLowerCase();
      var qNow = loadQ(); var cur = qNow.findIndex(function(x){ return x.id === id; });
      var nextIdx = (cur >= 0 ? cur + 1 : 0);
      if(nextIdx < qNow.length){
        var u = qNow[nextIdx].url || '#'; if(!/\.html($|\?)/i.test(u)) u += '.html';
        location.href = u;
      }
    }, Math.max(0, delayMs));
  }
  function startCountdown(startSeconds, title){
    ...
    var ov = ensureOverlay(title);
    setOverlayVisible(true); updateOverlayText(window.__sbqCountdownRemain, title);
    ...
    window.__sbqCountdownTimer = setInterval(function(){
      ...
      if(window.__sbqCountdownRemain <= 0){ clearInterval(window.__sbqCountdownTimer); window.__sbqCountdownTimer = null; }
      updateOverlayText(window.__sbqCountdownRemain, title);
    }, 1000);
  }

  function bindQueueMode(){
    var btn = $('#queue-mode-toggle'); if(!btn || btn.__sbqBound) return;
    btn.__sbqBound = true;
  ...
  function updateToggle(active){
    ...
    var cb = sw.querySelector('input[type="checkbox"]'); if(cb) cb.checked = !!active;
    sw.setAttribute('aria-pressed', active ? 'true' : 'false');
  }
  function bindToggle(){
    var sw = $('#sbq-toggle'); if(!sw || sw.__sbqBound) return;
    sw.__sbqBound = true;
    var cb = sw.querySelector('input[type="checkbox"]');
    if(cb){ cb.checked = isMode(); cb.addEventListener('change', function(){ setMode(!!cb.checked); }, true); }
  }
  ...
  function bindPrevNext(){
    ...
    if(prev && !prev.__sbqBound){
      ...
      prev.addEventListener('click', function(){ clearNavAndCountdown(); var i=getIdx(); var n=Math.max(i-1,0); var q=loadQ(); if(q.length) location.href=(q[n].url||'#').replace(/(\.html)?($|\?)/i,'.html$2'); }, true);
    }
    if(next && !next.__sbqBound){
      next.__sbqBound = true;
      next.addEventListener('click', function(){ clearNavAndCountdown(); var i=getIdx(); var q=loadQ(); var n=Math.min(i+1, q.length-1); if(q.length) location.href=(q[n].url||'#').replace(/(\.html)?($|\?)/i,'.html$2'); }, true);
    }
  }
  function bindClear(){
    var c = $('#queue-clear'); if(!c || c.__sbqBound) return;
    c.__sbqBound = true;
    c.addEventListener('click', function(){
      if(!loadQ().length) return;
      clearNavAndCountdown();
      if(confirm('Clear all stories from your queue?')){ saveQ([]); syncBadges(); }
    }, true);
  }
  function bindClose(){
    var x = $('.queue-menu-close'); if(!x || x.__sbqBound) return;
    x.__sbqBound = true;
    x.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); clearNavAndCountdown(); bodyOff('sbq-open'); }, true);
  }
  ...
  function bindButton(){
    ...
    var img = btn.querySelector('img'); if(img && window.SBQ_STILL_FIRST) img.src = window.SBQ_STILL_FIRST;
    ...
    btn.addEventListener('click', function(e){
      e.preventDefault(); e.stopPropagation();
      var open = isOpen();
      if(!open){ bodyOn('sbq-open'); revealToggleOnce(); setMode(true); }
      else     { bodyOff('sbq-open'); }
    }, true);
